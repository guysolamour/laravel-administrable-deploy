...
namespace Guysolamour\Administrable\Deploy\Console\Commands;
...
class DeployCommand extends Command
{
    ...
    {
        $files = $this->filesystem->files($this->getStubsPath(), true);

        foreach ($files as $file ) {
            $content = $this->filesystem->compliedFile($file->getRealPath(), true, $this->getParsedName());

            $this->filesystem->put(base_path() . DIRECTORY_SEPARATOR . $file->getRelativePathname(), $content);
        }
    }
    ...
    {
        return \Guysolamour\Administrable\Deploy\ServiceProvider::packagePath() . 'stubs';
    }
}
